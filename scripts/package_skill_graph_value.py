...
当前证据支持统一图输入和轻量评分；**不支持关系注意力已经带来增益**。
...
- 同一 RTX 2080 Ti 主机；模型常驻；同步 CUDA；每个配置重复测量5次，先取组内中位，再跨12组报中位/P95。
...
**有初步证据**：图接口输入的轻量模型能在此任务保持高Top-K保留质量，免去可选几何代理，
...
- 运行时代码基线：`{a.code_commit}`；模型各自记录训练源码与split哈希。
