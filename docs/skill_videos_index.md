# 技能演示视频索引（Skill Videos Index，原子 + 组合）
...
| `transport` | exec | 组合 | results/skill_videos/exec_transport.mp4 | 组合：持件沿航点搬运（move+持件校验） | 已录制 |
| `grasp` | exec | 组合 | results/skill_videos/exec_grasp.mp4 | 组合：完整抓取（detect→approach→grip_close→lift_verify） | 已录制 |
| `place` | exec | 组合 | results/skill_videos/exec_place.mp4 | 组合：搬运+对中+释放落位（transport→pre_align→descend→release） | 已录制 |
| `insert` | exec | 组合 | results/skill_videos/exec_insert.mp4 | 组合：把持件压入座面并保压（press 模式） | 已录制 |
...
| `approach` | trans | 组合 | results/skill_videos/trans_approach.mp4 | 组合：悬停+精降接近（move→descend，不闭合） | 已录制 |
| `return_home` | trans | 组合 | results/skill_videos/trans_return_home.mp4 | 组合：开爪+回 HOME（关节回扫） | 已录制 |
| `pull` | ext | 原子 | results/skill_videos/ext_pull.mp4 | 抓持后沿方向拖拽并释放 | 已录制 |
| `wipe` | ext | 原子 | results/skill_videos/ext_wipe.mp4 | 并指压面+力带扫掠擦拭 | 已录制 |
| `peg_insert` | ext | 组合 | results/skill_videos/ext_peg_insert.mp4 | 组合：RL/IL 学习策略插销（自动加载策略 checkpoint，含接触/卡滞/姿态反馈） | 已录制 |
