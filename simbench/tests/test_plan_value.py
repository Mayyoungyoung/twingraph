...
def example():
    obj = dict(position=[0.1, 0.2, 0.8], quaternion=[1, 0, 0, 0], geoms=[])
    obs = dict(
        robot=dict(joints=[0] * 7),
        objects={"part": obj},
        goals=[dict(manipulated="part", position=[0, 0, 0.85])],
    )
    payload = dict(
        id="candidate",
        part="part",
        start_state="snapshot",
        grasp=dict(id="grasp", xyz=[0, 0, 0.8], yaw=0.0, width=0.02, cost=123),
        path=dict(joints=[[0] * 7, [0.1] * 7], id="route", cost=321),
        control=dict(force=3.0),
        terminal=dict(xyz=[0, 0, 0.85]),
        steps=[dict(skill="move", params=dict(target=[0, 0, 0.8]))],
        status="necessary_pass",
        cost=987,
        bindings={},
        unknown=[],
    )
    c = SimpleNamespace(
        ...
        part="part",
        status="necessary_pass",
        to_dict=lambda: copy.deepcopy(payload),
    )
    suffix = [
        Call(
            "suffix",
            ...
            dict(
                part=argument("part"),
                target=Argument([0, 0, 0.85], "position", frame="world"),
            ),
            {"manipulated": "part"},
        )
    ]
...
def model():
    torch.manual_seed(9)
    return PlanValueNet(
        ModelConfig(width=32, layers=2, heads=4, dropout=0, vision=False)
    ).eval()
...
def test_prefix_cannot_see_suffix_even_indirectly():
    ...
    torch.testing.assert_close(
        a["prefix_logit"], b["prefix_logit"], atol=1e-6, rtol=1e-6
    )
...
def test_object_order_does_not_change_tokens():
    obs, p = example()
    obs["objects"]["other"] = dict(
        position=[0, 1, 0], quaternion=[1, 0, 0, 0], geoms=[]
    )
    before = encode_plan(obs, p)
...
def test_conditional_loss_ignores_failed_prefixes_and_counts_trials():
    ...
    loss = probability_loss(
        dict(prefix_logit=a, suffix_logit=b),
        torch.tensor([4, 4, 4]),
        torch.tensor([0, 4, 2]),
        torch.tensor([0, 3, 1]),
    )
...
def test_keep_loss_boundary_and_all_failure_groups():
    ...
    assert subset_metrics([0.0, 0.0], [0])["hit"] is None
...
def test_restore_preserves_free_joint_quaternion_bits():
    import mujoco
    ...
    joint = next(
        i
        for i in range(ctx.model.njnt)
        if ctx.model.jnt_type[i] == mujoco.mjtJoint.mjJNT_FREE
    )
    qa = ctx.model.jnt_qposadr[joint]
    # This tiny norm drift also occurs naturally during MuJoCo integration.
    ctx.data.qpos[qa + 3 : qa + 7] *= 1 + 1e-12
    before = ctx.snapshot()
    ctx.restore(before)
