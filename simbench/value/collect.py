...
def dump(path, data):
    ...
    temporary.write_text(json.dumps(plain(data), indent=2, allow_nan=False), encoding="utf-8")
...
def collect_group(seed, out, repeats=4, render=True):
    ...
    with (group / "steps.log").open("w", encoding="utf-8") as log, contextlib.redirect_stdout(log):
        ...
        inputs = dict(schema="twingraph.group.v1", group_id=spec.config_id, split_group=spec.config_id,
                      protocol=PROTOCOL, task=asdict(spec), observation=observed, images=images,
                      candidates=[p.to_dict() for p in plans], snapshot_sha256=fingerprint(session),
        ...
        np.savez_compressed(group / "initial_physics.npz", **snapshot["physics"]["data"])
        outcomes = []
        try:
            for repeat in range(repeats):
                rng = np.random.default_rng(np.random.SeedSequence([seed, repeat, 731]))
                trial = dict(repeat=repeat, friction_scale=float(rng.uniform(.85,1.15)),
                             actuator_gain_scale=float(rng.uniform(.985,1.015)))
                ...
                for plan in plans:
                    ...
                    session.ctx.model.actuator_gainprm[:] = base_gain * trial["actuator_gain_scale"]
                    session.ctx.model.actuator_biasprm[:] = base_bias * trial["actuator_gain_scale"]
                    trial_plan.prefix["start_state"] = fingerprint(session)
                    start_time = session.ctx.data.time
                    wall = time.perf_counter()
                    prefix, suffix, failed_stage, error, valid = False, None, "prefix", "", True
                    try:
                        ...
                        session.hold(.25)
                        session.call("inspect", part="pin_left", target=spec.target, tol=.0015)
                        suffix = True
                        failed_stage = None
                    ...
                    outcomes.append(dict(candidate_id=plan.id, trial=trial, valid=valid,
                                         ...
                                         failure_stage=failed_stage, error=error,
                                         sim_seconds=float(session.ctx.data.time-start_time),
                                         wall_seconds=time.perf_counter()-wall, recoveries=0,
                                         final_position=session.ctx.obj_pos("pin_left").tolist(),
                                         held=session.held, executed_steps=len(session.results)))
                    dump(group / "outcomes.json", dict(input_sha256=input_hash, trials=outcomes))
        finally:
            session.restore(snapshot)
        ...
        if invalid:
            raise RuntimeError(f"{len(invalid)} invalid trials; first: {invalid[0]['error']}")
        summary = dict(group_id=spec.config_id, seed=seed, candidates=len(plans), trials=len(outcomes),
                       prefix_successes=sum(r["prefix_success"] for r in outcomes),
                       full_successes=sum(r["full_success"] for r in outcomes),
                       wall_seconds=time.perf_counter()-started, input_sha256=input_hash,
...
def main():
    ...
    manifest = dict(schema="twingraph.dataset.v1", protocol=PROTOCOL, request=vars(a),
                    mujoco=mujoco.__version__, numpy=np.__version__, python=platform.python_version(),
                    seed_range=[a.start_seed,a.start_seed+a.groups], label_source="physical_mujoco_rollouts",
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=a.workers) as pool:
        futures = {pool.submit(collect_group, seed, str(out), a.repeats, not a.no_render): seed for seed in range(a.start_seed,a.start_seed+a.groups)}
        for f in concurrent.futures.as_completed(futures):
            ...
            dump(out / "manifest.json", {**manifest, "completed":sorted(summaries,key=lambda x:x["seed"]),"errors":errors})
    if errors:
        raise SystemExit(1)
