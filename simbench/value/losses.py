...
def probability_loss(output, trials, prefix_successes, full_successes):
    ...
    la = F.binary_cross_entropy_with_logits(
        output["prefix_logit"], a / n, reduction="none"
    )
    lb = F.binary_cross_entropy_with_logits(
        output["suffix_logit"], b / a.clamp_min(1), reduction="none"
    )
    # Prefix failures do not supervise the conditional suffix head.
    return (la * n).sum() / n.sum() + (lb * a).sum() / a.sum().clamp_min(1)


def keep_loss(scores, reference, group_ids, k=2, epsilon=0.1, margin=0.1):
    if k < 1:
