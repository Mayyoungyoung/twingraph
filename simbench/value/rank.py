...
class PlanRanker:
    def __init__(self, checkpoint, device="cpu"):
        ...
        self.device = device
        self.model = (
            PlanValueNet(ModelConfig(**saved["model_config"])).to(device).eval()
        )
    ...
    def rank(self, observation, plans, k=2, visual=None, batch_size=8):
        ...
        for start in range(0, len(eligible), batch_size):
            ...
            q = (
                result["q"]
                if self.saved["objective"] == "dual"
                else result["direct_logit"].sigmoid()
            )
            for i, plan in enumerate(batch_plans):
                scores.append(
                    dict(
                        ...
                        score=float(q[i]),
                        p_prefix=float(result["p_prefix"][i])
                        if self.saved["objective"] == "dual"
                        else None,
                        p_suffix_given_prefix=float(result["p_suffix"][i])
                        if self.saved["objective"] == "dual"
                        else None,
                        status=plan.status,
                        plan=plan.to_dict(),
        ...
        return dict(
            schema="twingraph.topk.v1",
            requested_k=k,
            returned_k=min(k, len(scores)),
            top_k=scores[:k],
            ranked=scores,
...
def main():
    ...
    p.add_argument(
        "--input", required=True, help="group inputs.json, with no outcome labels"
    )
    ...
    if ranker.model.config.vision:
        ...
        if cache.exists():
            ...
            if (
                str(features["input_sha256"]) != digest(inputs)
                or str(features["encoder"]) != ranker.saved["vision_encoder"]
            ):
            ...
            visual = FrozenVision(a.device).encode(
                [file.parent / f for f in inputs["images"]]
            )
    result = ranker.rank(
        inputs["observation"],
        [PlanIR.from_dict(c) for c in inputs["candidates"]],
        a.k,
        visual,
    )
    ...
    print(
        json.dumps(
            dict(
                output=a.out,
                selected=[r["candidate_id"] for r in result["top_k"]],
                scores=[r["score"] for r in result["top_k"]],
