...
class FrozenVision:
    ...
    def encode(self,paths):
        ...
        batch=torch.stack([self.transform(Image.open(p).convert("RGB")) for p in paths]).to(self.device)
...
def main():
    ...
    for complete in sorted(Path(a.data).glob("group_*/complete.json")):
        ...
        if output.exists():
            ...
            if str(old["encoder"])==ENCODER and str(old["input_sha256"])==digest(inputs):
                continue
        paths=[directory/f for f in inputs["images"]]
        features=encoder.encode(paths)
        np.savez_compressed(output,features=features,encoder=ENCODER,input_sha256=digest(inputs),
                            image_sha256=np.array([hashlib.sha256(f.read_bytes()).hexdigest() for f in paths]))
        count+=1
