...
class PlanValueNet(nn.Module):
    def __init__(self, config=None):
        ...
        self.number=nn.Sequential(nn.Linear(NUMERIC,c.width),nn.GELU(),nn.Linear(c.width,c.width))
        self.visual=nn.Linear(512,c.width) if c.vision else None
        self.visual_type=nn.Parameter(torch.zeros(1,1,c.width))
        self.cls=nn.Parameter(torch.randn(1,1,c.width)*.02)
        layer=nn.TransformerEncoderLayer(c.width,c.heads,c.width*4,c.dropout,activation="gelu",batch_first=True,norm_first=True)
        self.encoder=nn.TransformerEncoder(layer,c.layers,norm=nn.LayerNorm(c.width),enable_nested_tensor=False)
    ...
    def forward(self,batch):
        ...
        frequency=torch.exp(torch.arange(0,self.config.width,2,device=pos.device)*(-math.log(10000.)/self.config.width))
        ...
        x=self.key(batch["keys"])+self.category(batch["categories"])+self.status(batch["statuses"])+self.stage(batch["stages"])+self.number(batch["numbers"])+pe
        ...
        return dict(prefix_logit=a,suffix_logit=b,direct_logit=direct,
                    p_prefix=a.sigmoid(),p_suffix=b.sigmoid(),q=a.sigmoid()*b.sigmoid())
