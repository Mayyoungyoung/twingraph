...
VOCAB = 4096
NUMERIC = 24
EXCLUDE = {"id", "cost", "start_state", "bindings", "binding", "q_hover", "grasp_id", "prefix_id", "route_id", "terminal_id", "control_id", "grasp_epoch", "terminal_release_check", "observed_at"}


def bucket(text):
    return 1 + int.from_bytes(hashlib.blake2b(str(text).encode(), digest_size=4).digest(), "little") % (VOCAB-1)
...
def encode_plan(observation, plan):
    ...
    def add(key, value, stage, step=0, status="known", frame="", unit=""):
        ...
        elif isinstance(value, (list, tuple)) and all(isinstance(v,(int,float,bool)) for v in value):
            vals = np.array(value, dtype=np.float32)
        else:
        ...
        if len(vals) > 8:
            for i in range(0,len(vals),8):
                add(f"{key}/chunk/{i//8}", vals[i:i+8].tolist(), stage, step, status, frame, unit)
            return
        for i, scale in enumerate((1.,10.,100.)):
            numbers[i*8:i*8+len(vals)] = np.tanh(vals * scale)
        rows.append((bucket(f"{key}|{frame}|{unit}"),category,{"known":1,"deferred":2,"unknown":3}[status],stage,step,numbers))
    ...
    def visit(key, value, stage, step=0, status="known", frame="", unit=""):
        if isinstance(value, dict):
        ...
        elif isinstance(value, (list,tuple)) and any(isinstance(v,(list,tuple,dict)) for v in value):
            ...
            obj = objects[value]
            visit(key+"/object_pose", [*obj["position"],*obj["quaternion"]],stage,step,status,"world", "m,wxyz")
        else:
    ...
    for i, call in enumerate(plan.calls):
        ...
        for name, arg in sorted(call.arguments.items()):
            ...
            visit("argument/"+name,arg.value,a_stage,i+1,arg.status,arg.frame,arg.unit)
            if arg.source_call:
                add("producer_relative_index",[i-next(j for j,c in enumerate(plan.calls) if c.id==arg.source_call)],a_stage,i+1)
                add("producer_output",arg.source_output,a_stage,i+1)
    if not rows:
        raise ValueError("empty plan input")
    return dict(keys=np.array([r[0] for r in rows]), categories=np.array([r[1] for r in rows]),
                statuses=np.array([r[2] for r in rows]), stages=np.array([r[3] for r in rows]),
                positions=np.array([r[4] for r in rows]), numbers=np.stack([r[5] for r in rows]))
...
def sample_path(path, count):
    ...
    sampled=np.stack([np.interp(np.linspace(0,distance[-1],count),distance,points[:,i]) for i in range(points.shape[1])],axis=1)
...
def collate(encoded, visuals=None, device="cpu"):
    ...
    for key in ("keys","categories","statuses","stages","positions","numbers"):
        shape=(len(encoded),size,NUMERIC) if key=="numbers" else (len(encoded),size)
    ...
    batch["padding"]=torch.arange(size,device=device)[None,:]>=torch.tensor([len(x["keys"]) for x in encoded],device=device)[:,None]
