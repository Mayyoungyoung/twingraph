...
外部调用只需要 `observation + candidates`；任务目标保存在 `observation.goals`，每个候选 PlanIR 保存完整技能调用、参数和执行边界。共享注册表生成可执行图，价值模块输出整计划分数和 Top-K 原图，物理验证器消费其中同一份 PlanIR。
...
使用仓库原始滑台场景。机器人逐步装好滑块与端挡，形成检查点 2；或继续按现有生产配方装好左销，形成检查点 3。没有把已装配零件传送到成功状态。剩余候选分别完成“双销与手柄”或“右销与手柄”，含 58 / 40 条计划调用，两种长度都在训练和测试中出现。
...
| 划分 | 预先指定配置 | 到达评分检查点 | 前置失败 | 后续计划执行尝试 |
|---|---:|---:|---:|---:|
| 训练 | 12 | 10 | 2 | 160 |
| 验证 | 4 | 4 | 0 | 64 |
| 测试 | 8 | 6 | 2 | 96 |
