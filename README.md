...
执行层保留 **30 个底层组件**，包括控制步骤、计算模块和检查器，**不把它们视作 30 个平级任务技能**。拟采用的 `Observe / Move / Grasp / Place / Insert / Actuate / Inspect` 加恢复机制见 [架构设计](docs/architecture.md)。统一任务级接口、VLM 接入和新场景终态价值排序尚待实现。
...
验证记录见 [docs/evidence](docs/evidence)。历史小样本结果为完整装配扰动测试 8/8、插销策略新起点测试 12/12，适用条件写在任务文档中；它们不是现实机器人或新价值模块的性能成绩。
