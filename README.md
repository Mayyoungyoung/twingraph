...
**v5 状态与完整技能程序价值模块（正式实验进行中）**：[设计与监督目标](docs/value-v5-design.md)、[冻结实验协议](experiments/value_v5/protocol.json)、[运行指南](docs/value-v5-running.md)。输入统一为当前状态与完整候选程序，字段由技能接口自动编码；标签来自实际完整执行，不需要人工加权评分。候选从五部件均未装配的供料状态开始，模型粗筛后由数字孪生验证，再在独立目标仿真执行。模型、准确率与实测时间将在冻结评估完成后发布；当前不把开发预检作为正式结果。
